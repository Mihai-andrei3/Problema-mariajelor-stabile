...
(define (match person engagements pref1 pref2 queue)
  (let match-person ((pers person)
                     (it-eng engagements)
                     (favourites (get-pref-list pref1 person)) )

    (if  (null? favourites)  (cons (cons #f pers) it-eng )
        (let* ( (favourite (car favourites) )
                (p-list (get-pref-list pref2 favourite) )
                (fiance (get-partner it-eng favourite) )
               )
          (if (not (member favourite queue))
              (if (equal? fiance #f) (update-engagements it-eng favourite pers)
                (if (preferable? p-list pers fiance)
                 (match-person fiance (update-engagements it-eng favourite pers) (get-pref-list pref1 fiance))
                 (match-person pers it-eng (cdr favourites))))
...
(define (path-to-stability engagements mpref wpref queue)
   (let iteration ( (it-eng engagements) (q queue))
     (if (null? q)
        (if (null? it-eng) '()
         (if (or (member (caar it-eng)  (get-men mpref)) (member (get-partner it-eng (caar it-eng)) (get-women wpref)))
               (rev-eng it-eng)
               it-eng))
         (let* ( (person (car q))
                 (men (get-men mpref))
                 (woman (get-women wpref))
                 )
                
           (if (member person men)
               (if (null? it-eng) (iteration (match person it-eng mpref wpref q) (cdr q))
               (if (or (member (caar it-eng)  (get-men mpref)) (member (get-partner it-eng (caar it-eng)) (get-women wpref)))
                  (iteration (match person (rev-eng it-eng) mpref wpref q) (cdr q))        
                  (iteration (match person it-eng mpref wpref q) (cdr q))))
               (if (null? it-eng) (iteration (match person (rev-eng it-eng) wpref mpref q) (cdr q))
               (if (or (member (caar it-eng)  (get-men mpref)) (member (get-partner it-eng (caar it-eng)) (get-women wpref)))
                 (iteration (match person it-eng wpref mpref q) (cdr q))
                 (iteration (match person (rev-eng it-eng) wpref mpref q) (cdr q)))))))))
...
(define (delete-unstable engagements unstable)
  (if (null? engagements) '()
      (if (member (car engagements) unstable) (delete-unstable (cdr engagements) unstable)
          (cons (car engagements) (delete-unstable (cdr engagements) unstable))
)))

(define (update-stable-match engagements mpref wpref)
  (let* ((unstable (get-unstable-couples engagements mpref wpref))
         (new-eng (delete-unstable engagements unstable))
         (queue (get-couple-members unstable)) )
         
          (path-to-stability new-eng mpref wpref queue)
...
(define (build-stable-matches-stream pref-stream)
  (if (stream-empty? pref-stream) empty-stream
      (let* ((mpref0 (car (stream-first pref-stream)))
             (wpref0 (cdr (stream-first pref-stream)))
             (eng0 (gale-shapley mpref0 wpref0)))
        (let iteration ((stream (stream-rest pref-stream))
                         (eng eng0))
          (if (stream-empty? stream)
            empty-stream
              (let* ((mpref-i (car (stream-first stream)))
                     (wpref-i (cdr (stream-first stream)))
                     (new-eng (update-stable-match eng mpref-i wpref-i)))
                (stream-cons eng0 (stream-cons new-eng (iteration (stream-rest stream) new-eng)))))))
 ))
