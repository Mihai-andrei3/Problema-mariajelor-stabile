...
(define (get-partner engagements person)
 (if (equal? (find-first (lambda (d)(equal? (car d) person)) engagements) #f)
   #f
   (cdr (find-first (lambda (d)(equal? (car d) person)) engagements)))
  )
...
(define (change-first p L val)
 ( cond
    ((null? L) '())
    ((not (equal? (find-first p L) (car L))) (cons(car L) (change-first p (cdr L) val)))
    ((equal? (find-first p L) (car L )) (cons val (change-first null? (cdr L) val  ))))
  )
...
(define (update-engagements engagements p1 p2)
   (change-first (lambda (d)(equal? (car d) p1)) engagements (cons p1 p2)) )
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (if (not (equal? pref2 '()) )
      (if (and (preferable? (get-pref-list pref2 (car (car pref2))) p1 (get-partner engagements (car (car pref2))) ) (preferable?  p1-list (car (car pref2)) p2))
          #true
          (better-match-exists? p1 p2 p1-list (cdr pref2) engagements)
      )
      (if  (not (equal? pref2 '() ) )
          (better-match-exists? p1 p2 p1-list (cdr pref2) engagements)
          #false 
...
(define (rev-eng engagements)
  (map (lambda (pair) (cons (cdr pair) (car pair))) engagements))



(define (stable-match? engagements mpref wpref)
  (if (null? mpref) #t
   (if (equal? (better-match-exists? (caar mpref) (get-partner (rev-eng engagements) (caar mpref)) (get-pref-list mpref (caar mpref)) wpref engagements)  #t)
       #f
    (stable-match?  engagements(cdr mpref) wpref)  
   )))
